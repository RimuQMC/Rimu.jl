...
# Here, the numbering of the modes folds in the two spatial dimensions.  Use the utility
# function [`fock_to_cart`](@ref) to convert a Fock address to human-readable Cartesian
# quantum numbers for inspection.
fock_to_cart(addr, S)
...
# Then, use the utility function [`get_all_blocks`](@ref) to find all blocks. The blocks
...
# of each block and convert to [`DVec`](@ref)s.
