...
To support reproducibility, the metadata of the simulation report includes the the fields
- "random_seed": The random seed used (or `nothing` if no seed was used),
- "first_rand": the value of the first `UInt` random number generated by `solve` (or
    `nothing` if no seed was used), and
- "hash(1)": the output of `hash(1)`.

By comparing these values across two simulations, one can check whether the same random seed
and hash values were used, and whether multithreading was used. If the random seeds and hash
values are identical and multithreading was not used, the results of two simulations can be
