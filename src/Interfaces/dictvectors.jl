...
"""
    ...
    apply_operator!(::CompressionStrategy, working_memory, target, source, operator, boost=1)
...
`source`. The `working_memory` will be used as temporary storage.
...
For the operator to be applied without compressing the vector, set
`compress` to `Val(false)`. A [`CompressionStrategy`](@ref) can be passed instead to control
the applied compression. The `boost` argument is passed to [`apply_column!`](@ref) and
increases the number of spawns performed for a compatible `StochasticStyle(target)`.
...
function apply_operator!(
    working_memory, target, source, ham, boost=1, ::Val{C}=Val(true)
) where {C}
    if C
        compress = CompressionStrategy(StochasticStyle(target))
    else
        compress = NoCompression()
    end
    return apply_operator!(compress, working_memory, target, source, ham, boost)
end
function apply_operator!(
    CS::CompressionStrategy, working_memory, target, source, ham, boost=1
)
    v = localpart(source)
    ...
    if CS isa NoCompression
        names = spawn_names
        stats = spawn_stats
    else
        comp_names, _ = step_stats(CS)
        comp_stats = compress!(CS, target)
