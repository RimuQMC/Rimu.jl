...
"""
    apply_operator!(
        [::CompressionStrategy], working_memory, target, source, operator, boost=1
...
Optionally, a [`CompressionStrategy`](@ref) can be passed instead to control
the applied compression. Passing `NoCompression()` will disable compression.
The `boost` argument is passed to [`apply_column!`](@ref) and
...
"""
function apply_operator!(working_memory, target, source, ham, boost=1)
    compression = CompressionStrategy(StochasticStyle(target))
    return apply_operator!(compression, working_memory, target, source, ham, boost)
end
